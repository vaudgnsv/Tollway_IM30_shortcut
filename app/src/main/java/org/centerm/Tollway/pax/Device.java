...
public class Device {
    ...
    public static void beepOk() {
        MainApplication.getDal().getSys().beep(EBeepMode.FREQUENCE_LEVEL_3, 100);
        MainApplication.getDal().getSys().beep(EBeepMode.FREQUENCE_LEVEL_4, 100);
        MainApplication.getDal().getSys().beep(EBeepMode.FREQUENCE_LEVEL_5, 100);
    }
    ...
    public static void beepErr() {
        MainApplication.getDal().getSys().beep(EBeepMode.FREQUENCE_LEVEL_6, 200);
    }
    ...
    public static void beepPromt() {
        MainApplication.getDal().getSys().beep(EBeepMode.FREQUENCE_LEVEL_6, 50);
    }
 ...
 //    * @param tmkValue
    ...
    public static boolean writeTMK(byte[] tmkValue) {
        // write TMK
        try {
            MainApplication.getDal().getPed(EPedType.INTERNAL).writeKey(EPedKeyType.TLK, (byte) 0,
    ...
    public static boolean writeTPK(byte[] tpkValue, byte[] tpkKcv) {
        try {
            ...
            MainApplication.getDal().getPed(EPedType.INTERNAL).writeKey(EPedKeyType.TMK, Constants.INDEX_TMK,
    ...
    public static boolean writeTIKFuc(byte[] keyValue, byte[] ksn) {
        // write TIK
        try {
            MainApplication.getDal().getPed(EPedType.INTERNAL).writeTIK(Constants.INDEX_TIK, (byte) 0,
    ...
    public static byte[] getPinBlock(String panBlock) throws PedDevException {
        IPed ped = MainApplication.getDal().getPed(EPedType.INTERNAL);
