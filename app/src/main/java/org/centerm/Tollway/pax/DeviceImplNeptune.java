...
import org.centerm.Tollway.utility.TickTimer;
...
//    });
    ...
    public int pedVerifyCipherPin(final RsaPinKeyL2 rsaPinKeyIn, byte[] iccRespOut, byte mode) {
        ...
        try {
            //ped.setKeyboardLayoutLandscape(!ViewUtils.isScreenOrientationPortrait(ActivityStack.getInstance().top()));
            Log.d("kang","expectPinLen:" + expectPinLen);
...
//            int ret = detectOtherCard();
            ...
            //Log.i(TAG, "//Activity.getrReadType = " + ret);
            if (ret != EReaderType.DEFAULT.getEReaderType()) {
                return (byte) ret;
            } else {
                byte result = (byte) piccIsoCommandDevice(apduSend, apduRecv);
                if(result ==  4) {
...
//        long diff;
        ...
        try {
            ////Activity.prnTime("waveFlowBegin isoCommandByApdu strat time = ");
            ApduRespInfo resp = picc.isoCommandByApdu(iccSlot, send);
            ////Activity.prnTime("waveFlowBegin isoCommandByApdu time = ");
