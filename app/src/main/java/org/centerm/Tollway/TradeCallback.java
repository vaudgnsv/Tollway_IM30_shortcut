...
public class TradeCallback extends TransCallback {
    ...
    public TradeCallback(Activity activity, Dialog dialog) {
        this.activity = activity;
        this.promptDialog = dialog;
    }


    public static synchronized TradeCallback getInstance(Activity activty, Dialog dialog) {
        if (tradeCallback == null) {
            tradeCallback = new TradeCallback(activty, dialog);
        }
                            ...
                            activity.runOnUiThread(new Runnable() {
                                ...
                                public void run() {
                                    if (promptDialog == null) {
                                        ...
                                        promptDialog = Utility.getDialogAlert();
                                        Utility.customDialogAlert(Contextor.getInstance().getContext(),"Remove card...");
                                        promptDialog.show();
                                ...
                                public void run() {
                                    if (promptDialog == null) {
                                       ...
                                       // promptDialog.setTitleText(activity.getString(R.string.trans_sale));
                                        ...
                                        //CalculatePriceActivity.getinstance().customDialogCardError();
                                        promptDialog =  Utility.getDialogAlert();

                                    }
                                    promptDialog.setCancelable(false);
                                    Utility.customDialogAlert(Contextor.getInstance().getContext(), "please see phone...");
                                    //promptDialog.show();
                                    promptDialog.setCancelable(false);
